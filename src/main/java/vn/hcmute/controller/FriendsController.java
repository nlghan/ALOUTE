...
public class FriendsController {
	...
	public ResponseEntity<String> followUsers(@PathVariable long user2Id, HttpSession session) {
		...
		if (friend != null) {
			...
			// Xử lí thông báo
			String link = "/profile/" + user1.getUserID();
	...
	public List<UserInfoEntity> suggest(long idUsercurrent, long sizeList) {
		...
		List<UserInfoEntity> listUser = userInfoService.findAll();
		
		int i = 0;
		while (i < sizeList) {
			//int randomIndex = random.nextInt(listUser.size());
			int randomIndex = secureRandom.nextInt(listUser.size());

            UserInfoEntity user = listUser.get(randomIndex);

            Long userId = user.getUserID();
            
            if (userId != idUsercurrent) {
				FriendsEntity friend = friendsService.findByUser1IDAndUser2ID(idUsercurrent, userId);
				boolean shouldAdd = false;

				if (friend == null) {
					shouldAdd = userInfoService.findById(userId).isPresent() && !userList.contains(user);
				} else { 
					shouldAdd = !friend.isStatus() && !userList.contains(user);
				}
						  
				if (shouldAdd) { userList.add(user); i++; }
			}
		}
