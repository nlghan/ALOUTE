...
public class UserController {
	...
	public String showLogin(HttpServletRequest request, ModelMap model) {
		...
		// Thêm csrfToken để kiểm tra khi người dùng đăng nhập vào form và lưu nó trong session
	    ...
	    if (cookies != null) {
	        for (Cookie cookie : cookies) {
	            if ("email".equals(cookie.getName())) {
	                model.addAttribute("emailcookie", cookie.getValue());
	            } else if ("pass".equals(cookie.getName())) {
	                model.addAttribute("passcookie", cookie.getValue());
	            }
	        }
	    }
	...
	public String CheckLoginn(HttpServletResponse response,
			ModelMap model, @RequestParam("email") String Email,
			@RequestParam("password") String pass, HttpSession session,
			@ModelAttribute UserAcountModel user_account,
			@RequestParam("csrfToken") String formCsrfToken) {
		
		//Lấy csrf của session để kiểm tra
	    String sessionCsrfToken = (String) session.getAttribute("csrfToken");


	    if (!formCsrfToken.equals(sessionCsrfToken)) {
	...
	@GetMapping("/forgot")
	public String showFormMail(HttpServletRequest request, ModelMap model) {
		HttpSession session = request.getSession();
	    String csrfToken = UUID.randomUUID().toString();
	    session.setAttribute("csrfToken", csrfToken);
	    model.addAttribute("csrfToken", csrfToken);
	...
	public String Send(HttpServletRequest request, ModelMap model, @ModelAttribute("mail") EmailInfo emailInfo) {
	    ...
	    if (!formCsrfToken.equals(sessionCsrfToken)) {
	        return "redirect:forgot?error=csrf";
	    }
